...
class TestCustomHammingDistTopK(TestCase):
    ...
    def test_hamming_dist_top_k_mla_graph(self):
        ...
        npu_mode = torch.compile(npu_mode, backend=npu_backend, fullgraph=True, dynamic=False)
        npu_out = npu_mode(qhash.to(npu), khash_nope.to(npu), khash_rope_pad.to(npu), top_k.to(npu), seqlen.to(npu), chunk_size.to(npu), max_seq_len, sink, recent, support_offload, block_table.to(npu), mask.to(npu), indices.to(npu))
